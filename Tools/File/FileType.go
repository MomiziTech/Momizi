/*
 * @Author: NyanCatda
 * @Date: 2022-03-28 13:24:47
 * @LastEditTime: 2022-03-28 13:52:20
 * @LastEditors: NyanCatda
 * @Description: 文件类型判断
 * @FilePath: \Momizi\Tools\File\FileType.go
 */
package File
 ...
 * @param {[]byte} src 原始字节数组
 * @return {string} 返回前面结果字节的二进制
 */
...
func GetFileType(FileBody []byte) string {
	var fileType string
	fileCode := bytesToHexString(FileBody)
...
func CorrectFileType(Path string) (string, error) {
	// 打开文件
	File, err := NewFileReadWrite(Path, os.O_RDONLY)
	if err != nil {
		return "", err
	}
	defer File.Close()
	...
	// 读取文件
	FileBody, err := File.Read()
	if err != nil {
		return "", err
	}

	// 获取文件类型
	FileType := GetFileType([]byte(FileBody))
	...
	if FileType != "" {
		...
		// 去除文件后缀
		TrimExtPath := strings.TrimSuffix(Path, filepath.Ext(Path))

		err := Move(Path, TrimExtPath+FileType)
		if err != nil {
			return "", err
		}
		return TrimExtPath + FileType, nil
	}
	return Path, nil
}
